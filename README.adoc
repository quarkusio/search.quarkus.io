...
To update the localized site samples:

* Use the same list of guide references as was used in creating the main (`quarkusio-sample.zip`) sample.
* Run `QuarkusIOSample.main()` using your IDE,
with the project root as the current working directory,
and passing as first command-line argument the path to your clone of [lang-code].quarkus.io. repository and the `lang-code` as a second argument.
E.g. for a Japanese site the arguments will be `path/to/ja.quarkus.io ja`.
* Repeat this for all localized samples (`cn`, `es`, `ja`, `pt`).
