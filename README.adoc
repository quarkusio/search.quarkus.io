...
INDEXING_ERROR_REPORTING_TYPE=github-issue
----
...
* Production (`production` branch): https://console-openshift-console.apps.ospo-osci.z3b1.p1.openshiftapps.com/k8s/cluster/projects/prod-search-quarkus-io
* Staging (`main` branch): https://console-openshift-console.apps.ospo-osci.z3b1.p1.openshiftapps.com/k8s/cluster/projects/dev-search-quarkus-io
...
3. Config maps and secrets.
  `search-quarkus-io-config`::
    ...
    Besides that, configure a `GITHUB_OAUTH` environment variable and set it to
    a GitHub token that allows commenting/reopening/closing a GitHub issue.
    See `indexing.error-reporting.github-issue` configuration properties for more details.
